...
def unique(file_name1, file_name2):
	...
	#PART C PRINT
	for i in sorted(diff_name_for_class): #sort by name
		print "%s : %d" %(i,len(diff_name_for_class_net[i]))
...
def entropy(file_name):
	...
	#print total_entropy
def entropy_net(file_name):
	net_file = open(file_name, "r") #open file
	...
	for name in num_of_name:
		...
		entropy[name] = prob
		#print "%s\t%.5f" %(name,-prob*math.log(prob,2))
	descending_entropy = OrderedDict(sorted(entropy.items(), key=lambda k: k[1], reverse=True)) #sort by value in descending order.
	for i in descending_entropy:
		print "%s %s" %("entopy:",descending_entropy[i])
